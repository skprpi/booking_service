...
class CustomManager(BaseUserManager):
    ...
    def create_user(self, email, password=None, is_admin=False, name=None,
        ...
        user = self.model(
            email=email, name=name, surname=surname, break_time=break_time,
            is_admin=is_admin, timestamp=timestamp, role=role,
            is_staff=is_staff,
        )
...
class Lesson(models.Model):
    ...
    student_pk = models.ForeignKey(User, models.SET_NULL, blank=False,
                                   null=True, related_name='student_lesson')
    teacher_pk = models.ForeignKey(User, models.SET_NULL, blank=False,
                                   null=True, related_name='teacher_lesson')
    start_datetime = models.DateTimeField(blank=False)
    name = models.CharField(blank=False, max_length=50, default='Python')
...
class DisciplineDescription(models.Model):
    teacher_pk = models.ForeignKey(User, models.SET_NULL, blank=False,
                                   null=True,
                                   related_name='teacher_discipline_pk')
